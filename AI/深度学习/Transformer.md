...
Transformer 采用经典的编码器-解码器结构（常用于机器翻译等任务），但后续变体（如 BERT、GPT）只使用其中一部分。

**编码器**：由 N 个相同的层堆叠而成（通常 N=6）。每层包含两个子层：

1. 多头自注意力（Multi-Head Self-Attention）
2. 前馈神经网络（Feed-Forward Network, FFN）

每个子层后接残差连接 + 层归一化（Layer Normalization）。

**解码器**：同样由 N 个相同的层堆叠，每层包含三个子层：

1. 掩码多头自注意力（Masked Multi-Head Self-Attention）：防止解码时看到未来位置。
2. 编码器-解码器注意力（Cross-Attention）：让解码器关注编码器的输出。
3. 前馈神经网络
